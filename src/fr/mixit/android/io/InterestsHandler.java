...
public class InterestsHandler extends JsonHandler {
	...
	private static final String TAG = InterestsHandler.class.getSimpleName();
	...
	public void parse(ArrayList<JSONArray> entries, ContentResolver resolver) throws JSONException {
		...
		for (final JSONArray interests : entries) {
			if (DEBUG_MODE) {
				Log.d(TAG, "Retrieved " + interests.length() + " more interests entries.");
			}

			for (int i = 0; i < interests.length(); i++) {
				final JSONObject interest = interests.getJSONObject(i);
				final String id = interest.getString(TAG_ID);
	...
	private static boolean isInterestUpdated(Uri uri, JSONObject tag, ContentResolver resolver) throws JSONException {
		...
		try {
			...
			final String curName = cursor.getString(InterestsQuery.INTEREST_NAME).toLowerCase(Locale.getDefault()).trim();
			final String newName = tag.has(TAG_NAME) ? tag.getString(TAG_NAME).toLowerCase(Locale.getDefault()).trim() : curName;
