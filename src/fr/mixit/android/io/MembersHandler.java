...
public class MembersHandler extends JsonHandler {
	...
	public void parse(ArrayList<JSONArray> entries, ContentResolver resolver) throws JSONException {
		...
		int nbEntries = 0;
		for (final JSONArray members : entries) {
			Log.d(TAG, "Retrieved " + members.length() + " members entries.");
			nbEntries += members.length();

			parseMembers(members, resolver);
		}

		if (nbEntries > 0) {
			if (deleteMemberDataNotFoundInDB) {
				for (final Map.Entry<String, HashSet<String>> entry : memberInterestsIds.entrySet()) {
					...
					final HashSet<String> lostInterestIds = getLostIds(interestIds, memberInterestsUri, InterestsQuery.PROJECTION, InterestsQuery.INTEREST_ID,
							resolver);
				...
				for (final Map.Entry<String, HashSet<String>> entry : memberSharedLinksIds.entrySet()) {
					final String memberId = entry.getKey();
					...
					final HashSet<String> lostSharedLinkIds = getLostIds(sharedLinkIds, memberSharedLinksUri, SharedLinksQuery.PROJECTION,
							SharedLinksQuery.SHARED_LINK_ID, resolver);
				...
				for (final Map.Entry<String, HashSet<String>> entry : memberLinksIds.entrySet()) {
					final String memberId = entry.getKey();
					final HashSet<String> linkIds = entry.getValue();
					final Uri memberLinksUri = MixItContract.Members.buildLinksDirUri(memberId);
					final HashSet<String> lostLinkIds = getLostIds(linkIds, memberLinksUri, LinksQuery.PROJECTION, LinksQuery.LINK_ID, resolver);
				...
				for (final Map.Entry<String, HashSet<String>> entry : memberLinkersIds.entrySet()) {
					final String memberId = entry.getKey();
					...
					final HashSet<String> lostLinkerIds = getLostIds(linkerIds, memberLinkersUri, LinksQuery.PROJECTION, LinksQuery.LINKER_ID, resolver);
				...
				for (final Map.Entry<String, HashSet<String>> entry : memberSessionsIds.entrySet()) {
					final String speakerId = entry.getKey();
					final HashSet<String> sessionIds = entry.getValue();
					...
					for (final String lostSessionId : lostSessionIds) {
			...
			if (deleteMembersNotFoundInDB) {
				for (final String lostId : getLostIds(memberIds, MixItContract.Members.CONTENT_URI, MembersQuery.PROJECTION, MembersQuery.MEMBER_ID, resolver)) {
	...
	public void parseMembers(JSONArray entries, ContentResolver resolver) throws JSONException {
		for (int i = 0; i < entries.length(); i++) {
			final JSONObject member = entries.getJSONObject(i);
			final String id = member.getString(TAG_ID);
			memberIds.add(id);
			...
			if (newMember || memberUpdated) {
				...
				if (member.has(TAG_LASTNAME)) {
					...
					if (!TextUtils.isEmpty(lastname)) {
						...
						str.append(lastname.substring(0, 1).toUpperCase(Locale.getDefault()));
				...
				if (member.has(TAG_FIRSTNAME)) {
					...
					if (!TextUtils.isEmpty(firstname)) {
						...
						str.append(firstname.substring(0, 1).toUpperCase(Locale.getDefault()));
			...
			if (member.has(TAG_BADGES)) {
				final JSONArray batches = member.getJSONArray(TAG_BADGES);
				parseBadges(id, batches, resolver);
			}

			if (member.has(TAG_INTERESTS)) {
				final JSONArray interests = member.getJSONArray(TAG_INTERESTS);
				parseInterests(id, interests, resolver);
			}

			if (member.has(TAG_SHARED_LINKS)) {
				final JSONArray sharedLinks = member.getJSONArray(TAG_SHARED_LINKS);
				parseSharedLinks(id, sharedLinks, resolver);
			}

			if (member.has(TAG_LINKS)) {
				final JSONArray linkers = member.getJSONArray(TAG_LINKS);
				parseLinks(id, linkers, resolver);
			}

			if (member.has(TAG_LINKERS)) {
				final JSONArray linkers = member.getJSONArray(TAG_LINKERS);
				parseLinkers(id, linkers, resolver);
			}

			if (member.has(TAG_SESSIONS)) {
				final JSONArray sessions = member.getJSONArray(TAG_SESSIONS);
				parseSessions(id, sessions, resolver);
			}
		}
	}

	public void parseBadges(String memberId, JSONArray badges, ContentResolver resolver) throws JSONException {
		...
		for (int i = 0; i < badges.length(); i++) {
			...
			final String badgeId = badges.getString(i);
			// int badge = entries.getString(i);
			// Badge.valueOf(arg0)
			badgesIds.add(badgeId);

			final ContentProviderOperation ope = ContentProviderOperation.newInsert(memberBadgesUri) //
					.withValue(MixItDatabase.MembersBadges.BADGE_ID, badgeId) //
					.withValue(MixItDatabase.MembersBadges.MEMBER_ID, memberId) //
					.build();
			addOpeAndApplyBatch(ope, resolver, false);
		}
	}

	public void parseInterests(String memberId, JSONArray interests, ContentResolver resolver) throws JSONException {
		final Uri memberInterestsUri = MixItContract.Members.buildInterestsDirUri(memberId);
		...
		for (int j = 0; j < interests.length(); j++) {
			...
			final ContentProviderOperation ope = ContentProviderOperation.newInsert(memberInterestsUri) //
					.withValue(MixItDatabase.MembersInterests.INTEREST_ID, interestId) //
					.withValue(MixItDatabase.MembersInterests.MEMBER_ID, memberId) //
					.build();
			addOpeAndApplyBatch(ope, resolver, false);
		}
	...
	public void parseSessions(String memberId, JSONArray session, ContentResolver resolver) throws JSONException {
		...
		for (int j = 0; j < session.length(); j++) {
			...
			final String sessionId = String.format(Locale.getDefault(), FORMATER, id);
			sessionsIds.add(sessionId);

			final ContentProviderOperation ope = ContentProviderOperation.newInsert(speakerSessionsUri) //
					.withValue(MixItDatabase.SessionsSpeakers.SPEAKER_ID, memberId)//
					.withValue(MixItDatabase.SessionsSpeakers.SESSION_ID, sessionId)//
					.build();
			addOpeAndApplyBatch(ope, resolver, false);
		}
	...
	private void parseSharedLinks(String memberId, JSONArray sharedLinks, ContentResolver resolver) throws JSONException {
	...
	public void parseLinks(String memberId, JSONArray links, ContentResolver resolver) throws JSONException {
		final Uri memberLinksUri = MixItContract.Members.buildLinksDirUri(memberId);
		...
		for (int j = 0; j < links.length(); j++) {
			...
			final ContentProviderOperation ope = ContentProviderOperation.newInsert(memberLinksUri)//
					.withValue(MixItDatabase.MembersLinks.LINK_ID, linkId) //
					.withValue(MixItDatabase.MembersLinks.MEMBER_ID, memberId)//
					.build();
			addOpeAndApplyBatch(ope, resolver, false);
		}
	...
	public void parseLinkers(String memberId, JSONArray linkers, ContentResolver resolver) throws JSONException {
		final Uri memberLinkersUri = MixItContract.Members.buildLinkersDirUri(memberId);
		...
		for (int j = 0; j < linkers.length(); j++) {
			...
			final ContentProviderOperation ope = ContentProviderOperation.newInsert(memberLinkersUri)//
	...
	private static boolean isMemberUpdated(Uri uri, JSONObject member, ContentResolver resolver) throws JSONException {
		...
		try {
			...
			final String curLogin = !TextUtils.isEmpty(cursor.getString(MembersQuery.LOGIN)) ? cursor.getString(MembersQuery.LOGIN)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final String curEmail = !TextUtils.isEmpty(cursor.getString(MembersQuery.EMAIL)) ? cursor.getString(MembersQuery.EMAIL)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final String curFirstName = !TextUtils.isEmpty(cursor.getString(MembersQuery.FIRSTNAME)) ? cursor.getString(MembersQuery.FIRSTNAME)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final String curLastName = !TextUtils.isEmpty(cursor.getString(MembersQuery.LASTNAME)) ? cursor.getString(MembersQuery.LASTNAME)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final String curCompany = !TextUtils.isEmpty(cursor.getString(MembersQuery.COMPANY)) ? cursor.getString(MembersQuery.COMPANY)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final String curShortDesc = !TextUtils.isEmpty(cursor.getString(MembersQuery.SHORT_DESC)) ? cursor.getString(MembersQuery.SHORT_DESC)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final String curLongDesc = !TextUtils.isEmpty(cursor.getString(MembersQuery.LONG_DESC)) ? cursor.getString(MembersQuery.LONG_DESC)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final int curTicketRegistered = cursor.getInt(MembersQuery.TICKET_REGISTERED);
			final String curImageUrl = !TextUtils.isEmpty(cursor.getString(MembersQuery.IMAGE_URL)) ? cursor.getString(MembersQuery.IMAGE_URL)
					.toLowerCase(Locale.getDefault()).trim() : "";
			final int curNbConsult = cursor.getInt(MembersQuery.NB_CONSULT);
			final String newLogin = member.has(TAG_LOGIN) ? member.getString(TAG_LOGIN).toLowerCase(Locale.getDefault()).trim() : curLogin;
			final String newEmail = member.has(TAG_EMAIL) ? member.getString(TAG_EMAIL).toLowerCase(Locale.getDefault()).trim() : curEmail;
			final String newFirstName = member.has(TAG_FIRSTNAME) ? member.getString(TAG_FIRSTNAME).toLowerCase(Locale.getDefault()).trim() : curFirstName;
			final String newLastName = member.has(TAG_LASTNAME) ? member.getString(TAG_LASTNAME).toLowerCase(Locale.getDefault()).trim() : curLastName;
			final String newCompany = member.has(TAG_COMPANY) ? member.getString(TAG_COMPANY).toLowerCase(Locale.getDefault()).trim() : curCompany;
			final String newShortDesc = member.has(TAG_SHORT_DESC) ? member.getString(TAG_SHORT_DESC).toLowerCase(Locale.getDefault()).trim() : member
					.has(TAG_SHORT_DESCRIPTION) ? member.getString(TAG_SHORT_DESCRIPTION).toLowerCase(Locale.getDefault()).trim() : curShortDesc;
			final String newLongDesc = member.has(TAG_LONG_DESC) ? member.getString(TAG_LONG_DESC).toLowerCase(Locale.getDefault()).trim() : member
					.has(TAG_LONG_DESCRIPTION) ? member.getString(TAG_LONG_DESCRIPTION).toLowerCase(Locale.getDefault()).trim() : curLongDesc;
			...
			final String newImageUrl = member.has(TAG_IMAGE_URL) ? member.getString(TAG_IMAGE_URL).toLowerCase(Locale.getDefault()).trim() : curImageUrl;
	...
	private static boolean isSharedLinkUpdated(Uri uri, String memberId, JSONObject sharedLink, ContentResolver resolver) throws JSONException {
		final String[] selectionArgs = { memberId };
		...
		try {
			...
			final String curName = cursor.getString(SharedLinksQuery.NAME).toLowerCase(Locale.getDefault()).trim();
			final String curUrl = cursor.getString(SharedLinksQuery.URL).toLowerCase(Locale.getDefault()).trim();
	...
	private interface MembersQuery {
		String[] PROJECTION = { MixItContract.Members.MEMBER_ID, MixItContract.Members.LOGIN, MixItContract.Members.EMAIL, MixItContract.Members.FIRSTNAME,
				MixItContract.Members.LASTNAME, MixItContract.Members.COMPANY, MixItContract.Members.SHORT_DESC, MixItContract.Members.LONG_DESC,
				MixItContract.Members.TICKET_REGISTERED, MixItContract.Members.IMAGE_URL, MixItContract.Members.NB_CONSULT };
	...
	interface SharedLinksQuery {
		String[] PROJECTION = { MixItContract.SharedLinks.SHARED_LINK_ID, MixItContract.SharedLinks.ORDER_NUM, MixItContract.SharedLinks.NAME,
	...
	interface SessionsQuery {
		String[] PROJECTION = { MixItContract.Sessions.SESSION_ID, };
